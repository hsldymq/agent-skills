...
- Respect the project's decision authority and acceptance process. In conversational work, do not persist a discussion outcome as an accepted decision unless the user both explicitly confirms the concrete conclusion and explicitly requests the documentation update; when a repository defines another explicit acceptance authority or workflow, follow it instead. Partial agreement, a current leaning, an illustrative example, follow-up questions, or approval of one explanation is not acceptance. If the gate is unmet, keep the matter open or discussing and report any pending documentation sync without modifying project memory.
